...
export const createTransaction = (item_id, item_amount, item_variant, item_additional_price, payment_method, token) => {
    return async(dispatch) =>{
        console.log(item_id);
        const form = new URLSearchParams()
        item_id.map(value => 
            form.append('item_id', value)
        )
        item_amount.map(value => 
            form.append('item_amount', value)
        )
        item_variant.map(value => 
            form.append('item_variant', value)
        )
